...
impl RefpropFunctionLibrary {
    /// Performs a flash calculation given temperature, pressure, and bulk composition using the `TPFLSHdll` function.
    ///
    ...
    ///
    /// **Note:** For single-phase calculations, the subroutine `TPRHO` is much faster.
    ///
    /// # Parameters
    ///
    /// - `T`: Temperature [K]
    ...
    /// # References
    ///
    /// - [REFPROP Documentation - TPFLSHdll](https://pages.nist.gov/RefProp/documentation.html)
    pub fn tp_flash(T: f64, P: f64, z: &[f64]) -> Result<FlashOutput, RefpropError> {
