...
impl RefpropFunctionLibrary {
    /// Performs a flash calculation given pressure, entropy, and bulk composition using the `PSFLSHdll` function.
    ///
    ...
    ///
    /// **Note:** For single-phase calculations, the subroutine `PSFL1` is faster.
    ///
    /// # Parameters
    ///
    /// - `P`: Pressure [kPa]
    ...
    /// # References
    ///
    /// - [REFPROP Documentation - PSFLSHdll](https://pages.nist.gov/RefProp/documentation.html)
    pub fn ps_flash(P: f64, s: f64, z: &[f64]) -> Result<FlashOutput, RefpropError> {
