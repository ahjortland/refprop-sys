...
impl RefpropFunctionLibrary {
    /// Performs a flash calculation given temperature, quality, and bulk composition using the `TQFLSHdll` function.
    ///
    /// This method calculates various thermodynamic properties based on the provided temperature (`T`), vapor quality (`q`), and bulk composition (`z`).
    ///
    ...
    /// # References
    ///
    /// - [REFPROP Documentation - TQFLSHdll](https://pages.nist.gov/RefProp/documentation.html)
    pub fn tq_flash(
        T: f64,
        q: f64,
        z: &[f64],
...
mod tests {
    ...
    #[test]
    fn test_tq_flash() -> Result<(), RefpropError> {
        ...
        let T = 300.0;
        let q = 1.0;
        let result = RefpropFunctionLibrary::tq_flash(
            T,
            q,
            &z,
            Basis::Molar,
